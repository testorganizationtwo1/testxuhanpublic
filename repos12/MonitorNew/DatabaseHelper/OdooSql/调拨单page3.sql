select 
stock_picking.name as stock_picking_name,--单据号_FBillno									** 此页模板数据全部为空 => xh
--单据号_FTrantype
--保质期
stock_warehouse.id as stock_warehouse_id,--仓库_FNumber--调出仓库_FNumber
stock_warehouse.name as stock_warehouse_name,--仓库_FName--调出仓库_FName
stock_location.name as stock_location_name,--仓位_FName--调出仓位_FName
--仓位_FGroupName
--调出仓库_FNumber
--调出仓库_FName
--调出仓位_FName
...
--辅助属性_FName
--辅助属性_FClassName
res_partner.id as res_partner_id,--供应商_FNumber
res_partner.commercial_company_name as res_partner_commercial_company_name,--供应商_FName
stock_move_line.lot_name as stock_move_line_lot_name,--批号
--生产采购日期
--是否有效
product_product.material_code as product_product_material_code,--物料_FNumber
product_template.name as product_template_name,--物料_FName
product_product.product_model as product_product_product_model,--物料_FModel
--stock_production_lot.lot_ids as stock_production_lot_lot_ids,--序列号         ***   =>xh
--序列号ID
--序列号规则ID
stock_picking.state as stock_picking_state--状态
--最后SNListID
--最后单据类别
from stock_picking 
