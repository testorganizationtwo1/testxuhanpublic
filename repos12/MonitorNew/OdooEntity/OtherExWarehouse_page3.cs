...
{   
    ...
    /// 其他出库
    /// </summary>
    public class OtherExWarehouse_page3
    {
        ...
        public List<OtherExWarehouse_page3> GetEntity(DataTable dt)
        {
            List<OtherExWarehouse_page3> list = new List<OtherExWarehouse_page3>();
            ...
            {  //oew.stock_picking_date_done = Convert.ToDateTime (dt.Rows[i]["date_done"] == DBNull.Value ? DateTime.Now.AddYears(-10) : Convert.ToDateTime(dt.Rows[i]["date_done"])); //修改
                OtherExWarehouse_page3 oew = new OtherExWarehouse_page3();
