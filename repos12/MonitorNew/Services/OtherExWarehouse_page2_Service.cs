...
namespace MonitorNew.Services
{
    ...
    /// 其他出库page2数据交互
    /// </summary>
    public class OtherExWarehouse_page2_Service : BaseService
    {
        public static string InsertOtherExWarehouse_page2()
        {
            // 1、查询odoo  其他出库page2.sql
            Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->正在查询OdooHelper().GetOtherExWarehouse_page2()" + DateTime.Now);
            List<OtherExWarehouse_page2> dt = new OdooHelper().GetOtherExWarehouse_page2();
            if (dt == null)
                return "no,没有查到odoo其他出库page2数据";
            Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->查询OdooHelper().GetOtherExWarehouse_page2()结束" + DateTime.Now);

            Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->循环插入金蝶开始" + DateTime.Now);
            for (int i = 0; i < dt.Count; i++)
            {
                OtherExWarehouse_page2 item = dt[i];
                #region 循环插入金蝶
                #region 获取仓库Id的代码
                Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->正在插入仓库表 t_Stock FName=" + item.stock_warehouse_name + DateTime.Now);
                ...
                Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->仓库表 t_Stock 插入结束 FName=" + item.stock_warehouse_name + DateTime.Now);
                #endregion

                #region 获取物料Id的代码
                Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->正在插入物料表 t_ICItemCore FNumber=" + item.default_code + DateTime.Now);
                ...
                Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->物料表 t_ICItemCore 插入结束 FNumber=" + item.default_code + DateTime.Now);
                #endregion

                #region 获取单位Id的代码
                Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->正在插入计量单位表 t_MeasureUnit FName" + item.uom_uom_name + DateTime.Now);
                int FUnitID = 0;
                ...
                Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->计量单位表 t_MeasureUnit  插入结束 FName=" + item.uom_uom_name + DateTime.Now);
                #endregion

                #region 获取仓位Id的代码
                Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->正在插入仓位表 t_StockPlace FName" + item.stock_location_name + DateTime.Now);
                int FDCSPID = 0;//仓位Id
                ...
                Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->仓位表 插入结束 t_StockPlace FName" + item.stock_location_name + DateTime.Now);

                #endregion

                #region 获取外销订单Id的代码
                Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->正在外销订单表 SEOrder ******" + "******" + DateTime.Now);
                int SEOrder_FInterID = 0;
                object SEOrderSearchByFNameResult = SearchByColumnName("FInterID", "SEOrder", "******", "******");
                if (string.IsNullOrEmpty("******") || SEOrderSearchByFNameResult == null)
                {
                    #region 此外销订单在金蝶中不存在，以下为将此外销订单插入金蝶的代码
                    SEOrder sEOrder_entity = new SEOrder()
                    {
                        //FBrNo = ,
                        FInterID = new SqlHelper().GetMaxId("FInterID", "SEOrder"),
                    ...
                    new SqlHelper().SqlPour(SEOrderFormat.sql_SEOrder(sEOrder_entity), null);
                    SEOrder_FInterID = sEOrder_entity.FInterID;
                    Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->外销订单表 插入结束  ******" + "******" + DateTime.Now);
                    #endregion
                }
                if (SEOrder_FInterID == 0 && SEOrderSearchByFNameResult != null)
                {
                    SEOrder_FInterID = Convert.ToInt32(SEOrderSearchByFNameResult);
                }
                #endregion
                ...
                #region 获取辅助资料Id的代码
                Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->正在插入辅助资料表 t_SubMessage FName=" + "********" + DateTime.Now);
                ...
                Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->辅助资料表 插入结束  t_SubMessage FName=" + "********" + DateTime.Now);
                #endregion
                ...
                {
                    #region 此供应商在金蝶中不存在，以下为将此供应商插入金蝶的代码
                    Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->正在插入供应商表 t_Supplier FName=" + item.res_partner_commercial_company_name + DateTime.Now);
                    ...
                    Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->供应商表 插入结束 t_Supplier FName=" + item.res_partner_commercial_company_name + DateTime.Now);
                    #endregion
                }
                ...
                Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->正在插入出入库单据分录表 ICStockBillEntry" + DateTime.Now);
                ...
                ICStockBillEntry iCStockBillEntry = new ICStockBillEntry()
                {
                    ...
                    //FOrderEntryID = item.,
                    FOrderInterID = SEOrder_FInterID,
                    //FEntryID = ,
                    //FInterID = ,
                ...
                Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->出入库单据分录表 ICStockBillEntry 插入结束" + DateTime.Now);

                // (2) 插入金蝶 t_Stock表 *** ====顶端已判断，按需插入

                // (3) 插入金蝶t_ICItemCore表 *** ====顶端已判断，按需插入

                // (4) 插入金蝶t_MeasureUnit表 *** ====顶端已判断，按需插入

                // (5) 插入金蝶t_StockPlace表 *** ====顶端已判断，按需插入

                // (6) 插入金蝶 SEOrder 表 *** ====顶端已判断，按需插入
                ...
                //(10) 插入金蝶 t_SubMessage 表 *** ====顶端已判断，按需插入

                // (11) 插入金蝶 t_Supplier 表 *** ====顶端已判断，按需插入

                // (12) 插入金蝶 t_StockPlaceGroup 表 *** ====顶端已判断，按需插入


                #endregion
                Program.mf.ListBoxKingdeeTaskList.Items.Add("其他出库page2->循环插入金蝶结束" + DateTime.Now);
